...
function Library:CreateWindow(Config)
    local Title = Config.Title or "Nomade V2"
    
    -- Destroi UI antiga para não duplicar
    for _, v in pairs(CoreGui:GetChildren()) do
        if v.Name == "NomadeUI" then v:Destroy() end
    end

    local ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = "NomadeUI"
    ...
    local UIStroke = Instance.new("UIStroke") UIStroke.Color = Color3.fromRGB(255, 60, 60) UIStroke.Thickness = 1 UIStroke.Parent = MainFrame
    ...
    function Window:Tab(name)
        ...
        local PageList = Instance.new("UIListLayout") PageList.Padding = UDim.new(0, 8) PageList.HorizontalAlignment = Enum.HorizontalAlignment.Center PageList.Parent = PageFrame
        local PagePad = Instance.new("UIPadding") PagePad.PaddingTop = UDim.new(0,5) PagePad.Parent = PageFrame

        TabBtn.MouseButton1Click:Connect(function()
            -- Resetar Tabs
            for _, v in pairs(TabContainer:GetChildren()) do
                if v:IsA("TextButton") then
                    TweenService:Create(v, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(35,35,40), TextColor3 = Color3.fromRGB(150,150,150)}):Play()
                end
            end
            for _, v in pairs(Pages:GetChildren()) do v.Visible = false end
            
            -- Ativar Atual
            TweenService:Create(TabBtn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(255, 60, 60), TextColor3 = Color3.fromRGB(255,255,255)}):Play()
        ...
        function TabFunctions:Button(text, callback)
            ...
            Btn.MouseButton1Click:Connect(function()
                TweenService:Create(Btn, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(255,60,60)}):Play()
                wait(0.1)
        ...
        function TabFunctions:Toggle(text, default, callback)
            ...
            Switch.BackgroundColor3 = default and Color3.fromRGB(255,60,60) or Color3.fromRGB(50,50,55)
            Switch.Parent = Tgl
            ...
            Tgl.MouseButton1Click:Connect(function()
                active = not active
                local targetColor = active and Color3.fromRGB(255,60,60) or Color3.fromRGB(50,50,55)
